...
public class Asiakkaat extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pathInfo==null) { //Haetaan kaikki asiakkaat 
			...
			String asiakas_idSTR = pathInfo.replace("/haeyksi/", ""); //poistetaan polusta "/haeyksi/", j‰ljelle j‰‰ asiakas_id	
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(dao.lisaaAsiakas(asiakas)){ //metodi palauttaa true/false
			out.println("{\"response\":1}");  //Asiakkaat lis‰‰minen onnistui {"response":1}
		}else{
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		JSONObject jsonObj = new JsonStrToObj().convert(request); //Muutetaan kutsun mukana tuleva json-string json-objektiksi			
		String asiakas_idSTR = jsonObj.getString("asiakas_id");
		int asiakas_id=Integer.parseInt(asiakas_idSTR);
		Asiakas asiakas= new Asiakas();
		asiakas.setEtunimi(jsonObj.getString("etunimi"));
		asiakas.setSukunimi(jsonObj.getString("sukunimi"));
